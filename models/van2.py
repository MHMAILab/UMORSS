...
class VAN(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.unc_module = UncertaintyNetwork(in_channels=embed_dims[3])
    ...
    def forward(self, x):
        ...
        logits = self.head(x)
        unc = self.unc_module(x).flatten()

        return logits,unc,x

    def result_with_features(self, x):
...
@register_model
def van_tiny2(pretrained=False, **kwargs):
    model = VAN(
    ...
    if pretrained:
        model = load_model_weights(model, "van_tiny2", kwargs)
    return model
...
def van_base(pretrained=False, **kwargs):
    ...
    model.default_cfg = _cfg()
    if pretrained:
        print("load weights from van_base pretrained")
